...
export class WePayService {
  ...
  async getPlatformCertificates(
    ...
    const ret = await axios.get<{ data: CertificateResult[] }>(url, {
  ...
  async fapiaoCallback(
    ...
    req: Request,
    res: Response,
  ): Promise<FapiaoNotifyResult> {
    return this.paidCallback(
      certs,
      apiKey,
      req,
      res,
    ) as unknown as Promise<FapiaoNotifyResult>;
  }
  ...
  async fapiaoCallbackOfPartner(
    ...
    req: Request,
    res: Response,
  ): Promise<FapiaoNotifyResultOfPartner> {
    return this.fapiaoCallback(
      certs,
      apiKey,
      req,
      res,
    ) as unknown as Promise<FapiaoNotifyResultOfPartner>;
  }
  ...
  async refundedCallback(
    ...
    return this.paidCallback(
      ...
      req,
      res,
    ) as unknown as Promise<RefundNotifyResult>;
  }
  ...
  async refundedCallbackOfPartner(
    ...
    req: Request,
    res: Response,
  ): Promise<RefundNotifyResultOfPartner> {
    return this.paidCallback(
      certs,
      apiKey,
      req,
      res,
    ) as unknown as Promise<RefundNotifyResultOfPartner>;
  }
  ...
  async callbackHandlerForExpress<T>(
    ...
    const { fail, result, callbackBody } = await this.callbackHandler<T>(
      certs,
      apiKey,
      req.headers as SignatureHeaders,
      rawBody,
    );
  ...
  async callbackHandler<T>(
    ...
    if (publicKey) {
      ...
      if (verified) {
        ...
        result = this.decryptCipherText<T>(
          apiKey,
          resource.ciphertext,
          resource.associated_data,
          resource.nonce,
  ...
  getCallbackSignatureParameters(headers: SignatureHeaders) {
    ...
    const timestamp =
      headers['Wechatpay-Timestamp'] || headers['wechatpay-timestamp'];
  ...
  async sendRedPack(
    ...
    const url = `${this.API_ROOT}/mmpaymkttransfers/${
      group ? 'sendgroupredpack' : 'sendredpack'
    }`;
    const nonceStr = createNonceStr();
    const dataObj = {
      // eslint-disable-next-line camelcase
    ...
    if (group) {
      ...
      (dataObj as any).amt_type = {
        cValue: (redPack as GroupRedPackData).amtType || 'ALL_RAND',
      };
    }
   ...
   * @param prepayId String APP下单生成的prepay_id
   * @param privateKey String 微信支付私钥
   * @returns MiniProgramPaymentParameters
